...
import ycms.component.controller.ManagedTextTransferProcessController;
...
/**
 * Зрно са транзицијама текстова између разних заједничких извора података.  
 * @author VM
 * @version 1.0 
 */
public class ClipboardTextCollectionBean implements ycms.component.general.Text, TextCollection, AjaxExecutable{
	private static final long serialVersionUID = 2614997588658686144L;
	private ManagedTextTransferProcessController transferProccessController = new ManagedTextTransferProcessController(); 
	private DataSourceSchema dataSource = new DataSourceSchema(); 
	private DataSourceSchema dataDestination = new DataSourceSchema(); 
	...
	public ManagedTextTransferProcessController getTransferProccessController() {
		return transferProccessController;
	}
	...
	public DataSourceSchema getDataDestination() {
	...
	public void register(HttpSession session, String beanName) {
		...
		if(this.dataSource.getDataSourceValue() == DataSourceValue.JAVA_BUFFER)    this.transferProccessController.setSource(listEngine);
		if(this.dataSource.getDataSourceValue() == DataSourceValue.MONGO_DATABASE) this.transferProccessController.setSource(mongoEngine);
		if(this.dataDestination.getDataSourceValue() == DataSourceValue.JAVA_BUFFER)    this.transferProccessController.setDestination(listEngine);
		if(this.dataDestination.getDataSourceValue() == DataSourceValue.MONGO_DATABASE) this.transferProccessController.setDestination(mongoEngine);
	}
	...
	public void dataSourceJavaBuffer() {
		...
		transferProccessController.setSource(listEngine);
	}
	
	public void dataSourceMongoDatabase() {
		resetSelected();
		dataSource.setDataSourceValue(DataSourceValue.MONGO_DATABASE);
		transferProccessController.setSource(mongoEngine);
	}
	
	public void dataSourceJavaBufferDestination() {
		dataDestination.setDataSourceValue(DataSourceValue.JAVA_BUFFER); 
		transferProccessController.setDestination(listEngine);
	}
	
	public void dataSourceMongoDatabaseDestination() {
		dataDestination.setDataSourceValue(DataSourceValue.MONGO_DATABASE);
		transferProccessController.setDestination(mongoEngine);
	}
	...
	public boolean isDataSourceJavaBufferDestination() {
		return dataDestination.getDataSourceValue() == DataSourceValue.JAVA_BUFFER; 
	}
	
	public boolean isDataSourceMongoDatabaseDestination() {
		return dataDestination.getDataSourceValue() == DataSourceValue.MONGO_DATABASE;
	}
	...
	public void front() {
		...
		if(selectedId.trim().length()==0) return; 
		textBean.setText(selectedText.text());
	}
	...
	public void accept() {
		...
		if(selectedText==null) return;
		if(selectedId.trim().length()==0) return;
		selectedText.setText(textBean.getText());
	}
}
